...
> **iPhone Action Button + iOS Shortcuts + Google Apps Script (GAS)**

아이폰 15 Pro 이상의 **액션 버튼(Action Button)**을 활용하여 단 1초 만에 소비 내역을 구글 스프레드시트에 기록하고, 실시간 대시보드로 시각화하는 자동화 시스템입니다.

## 🚀 Key Features
- **One-Tap Logging**: 아이폰 액션 버튼을 통해 앱 실행 없이 즉시 지출 입력.
...
## 🏗 System Architecture
1. **Input**: 사용자가 아이폰 액션 버튼을 눌러 "항목 금액" 입력.
...
4. **Action Button**: 아이폰 설정에서 해당 단축어를 액션 버튼으로 지정합니다.
